...
## 2026-08-13 配送二级区域与边界

- checkout 在大区之后新增二级区域选择，二级区域及地图多边形统一存放在 `local_pallet.delivery_subareas`；订单只保存 `subarea_id` 和后端确认过的 `subarea_name` 快照，不把多边形复制进订单表。
- 后端是二级区域的唯一可信来源：前端只提交 `subarea_id`，后端校验该区域启用且属于所选大区后再写订单。用户选择 `Autre quartier` 时提交独立标记，订单保存 `subarea_id=NULL, subarea_name='Autre quartier'`。
- 不回填历史订单，也不根据历史地址或坐标推测二级区域。历史订单保持 `subarea_id=NULL, subarea_name=NULL`；只保证新订单的数据完整且可区分。
- 一级大区和二级区域都写入本机长期缓存，并绑定为同一组选择。下次进入 checkout 时，如果缓存的大区和二级区域仍在当前启用列表中，直接恢复两级区域并进入已预填的个人信息页；切换一级大区时清除旧二级缓存，后台删除或停用旧二级区域时重新要求选择。
- 新订单选了具体二级区域后，自动定位必须受该区域多边形约束：区域内的用户手动标点直接保留；否则使用“地址 + 二级区域 + 一级大区 + Abidjan”搜索，并只接受多边形内的候选结果。搜索无结果、报错或所有结果都越界时，统一使用二级区域中心点，不能退回一级大区中心或采用 Google 第一个越界结果。
- `Autre quartier` 也作为二级选择写入缓存，但订单仍保存 `subarea_id=NULL`。App、旧网页、`Autre quartier` 等只有一级区域的订单继续先做坐标反查、地址搜索和大区一致性验证：可靠的原坐标直接保留，地址搜索得到且反查命中已配置阿比让大区的坐标才采用；只有搜索失败、无法验证或地址过泛时才回落一级区域中心点，并设为定位检查通过，不进入人工确认。二级区域配置缺失时也回落一级中心点。`Attécoubé` 是独立大区，不再映射为 `Yopougon`。
...
- 误判修复保留最少5字符规则：单词地址与已知地名只差一次拼写或相邻字母颠倒时放行；具有已知区域加地址标记，或同时含多个 `rue/pharmacie/école/boutique/cité` 等结构的详细地址，不再进入乱码判定。
- 兼容特殊 Unicode 字体：前后端在校验前用兼容分解把数学粗体、等宽体等字符还原为普通字母，`𝙀𝙭𝙥é𝙙𝙞𝙩𝙞𝙤𝙣 à 𝙢𝙖𝙣...`、`𝚈𝚘𝚙𝚘𝚞𝚐𝚘𝚗...` 不再因归一化后为空而被误判为过短。
- 最近部署窗口发现的 4 单高置信度漏放使用精确值补丁拦截：`Ad1of2`、`33LAM` 和同一号码两次填写的 `nnkonesamba06kone.com.com`；不扩张成通用“字母加数字”规则，避免误伤本地地名和门牌写法。
