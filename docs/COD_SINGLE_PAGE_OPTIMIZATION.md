...
## 2026-08-18 网页前端撤回二级区域

- 网页 checkout 只撤回前端二级区域功能，后端、数据库字段、70 个二级区域配置和管理端定位能力全部保留，方便以后需要时重新启用；本次不修改任何后端仓库。
- 当前 G 组恢复到二级区域上线前的单页流程：用户选择一级大区后立即进入资料页；本机只恢复一级大区和同大区的手动标点缓存，不再展示或要求选择二级区域。
- 单品和组合品网页订单只提交 `district_id`、地址和坐标，不提交 `subarea_id` 或 `subarea_is_other`。后端按兼容的无二级区域合同保存订单，`subarea_id/subarea_name` 为空。
- 只有一级大区的订单继续沿用原定位流程：先检查已有坐标、反查结果和地址搜索；可靠坐标保留，低置信度或搜索失败才使用一级大区中心点并通过定位检查，不进入人工确认。
- 2026-08-13 上线的二级区域前端提交 `8735902` 和缓存提交 `d9beb16` 在网页端反向撤回；未来如重新启用，应重新评估是否强制展示，并重新验证缓存、埋点和下单转化率。
