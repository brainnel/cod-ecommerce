...
- 已有大区缓存的设备进入 checkout 后直接选择该大区下的二级区域；选完继续进入已预填的个人信息页。二级区域初期不做跨订单自动选择，避免用户换收货地点时沿用错误区域。
...
- `Autre quartier` 没有具体二级区域 ID，继续沿用原一级大区定位逻辑；`Attécoubé` 是独立大区，不再映射为 `Yopougon`。
